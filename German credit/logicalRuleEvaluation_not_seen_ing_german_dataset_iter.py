...
dataset[9]=dataset[9].map({0:'B',1: 'G'})
...
g=sns.scatterplot(ax=ax,x=dataset[1],y=dataset[0],alpha=0.6, hue=dataset[9],s=70)
g.get_legend().remove()
...
def getCorrelation(x, arraySet):
    ...
    dataset = pd.read_csv(other_data_name+'/german_df.csv', sep='\t', index_col=0)+1e-3
...
def getSpecies(y_pred,acc_rule, x):
    ...
    if num_rules==1:
        ...
        idx_toDecide=np.array(idx_toDecide).reshape(-1,10)
...
dataset = pd.read_csv(other_data_name+'/german_df.csv', sep='\t', index_col=0)
corrmat=dataset.corr()
corrAtrr=np.array(corrmat.iloc[:-1,[-1]]).reshape(1,-1)
indexes=np.array(sorted(corrAtrr.reshape(-1,1), key=lambda x: abs(x), reverse=True))
...
max_acc=np.load(other_data_name+'/german_test_acc_max.npy', allow_pickle=(True))  
notsignificant=np.load(other_data_name+'/german_not_signigicant_B.npy',allow_pickle=(True)).astype('float32')  
...
notsignificant=np.load(other_data_name+'/german_not_signigicant_F.npy',allow_pickle=(True)).astype('float32')  
...
# np.save(other_data_name+'/german_test_acc_max.npy',max_acc)  
# np.save(other_data_name+'/german_not_signigicant_B.npy',notsignificant)    


# np.save(other_data_name+'/german_not_signigicant_F.npy',notsignificant)    
