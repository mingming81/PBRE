...
# %%
dataset = pd.read_csv('./../v3/data2/german_df.csv', sep='\t', index_col=0)
    ...
    for i in range(1,fullSet1.shape[0]):
        ...
        if toCombined[idx][0,0][0,1]<=combined[idx][0,0][0,2]:
            ...
            if fullSet2[0,0]==None:
                ...
                fullSet2=copy.deepcopy(combined.reshape(-1,10))
                if toAddIdx[0,0]!=None:
                    for toi in toAddIdx[0]:
                        toAdd=fullSet1[toi]
                        toAdd[idx]=combined[idx]
                        fullSet2=np.concatenate((fullSet2,toAdd.reshape(-1,10)),axis=0)
                
            else:
                
                fullSet2=np.concatenate((fullSet2,combined.reshape(-1,10)),axis=0)
                if toAddIdx[0,0]!=None:
                    for toi in toAddIdx[0]:
                        toAdd=fullSet1[toi]
                        toAdd[idx]=combined[idx]
        ...
        for j in range(i+1,lenTotal):
            ...
            if i!=j:
                ...
                # user state  
                    ...
                    fullSet1=np.concatenate((fullSet1, rule2.reshape(-1,10)),axis=0)
...
class Node:
    ...
    def checkNode(self,rule,checkType):
        ...
        if checkType!=0:
            ...
            importAtt=np.array([i for i in range(1,10)])
