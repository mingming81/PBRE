...
dataset = pd.read_csv(other_data_name+'/heart_clearned_df.csv', sep='\t')
...
def calculateEpsilon(checkType,dataset):
    ...
    values=sorted(dataset[key_val[checkType-1]].values, key=lambda x: x)
    
    epsilon=0
    
    for i in range(dataset[key_val[checkType-1]].values.shape[0]-1):
        
        diff+=abs(values[i+1]-values[i])
        epsilon=math.ceil(diff/(dataset[key_val[checkType-1]].values.shape[0]-1))
...
def combineRules(fullSet1,idx):
    ...
    for i in range(1,fullSet1.shape[0]):
        ...
        if toCombined[idx][0,0][0,1]<=combined[idx][0,0][0,2]:
            ...
            mean=(combined[idx][0,1]*combined[idx][0,0][0,0][0,0]+toCombined[idx][0,1]*toCombined[idx][0,0][0,0][0,0])/(combined[idx][0,1]+toCombined[idx][0,1])

            d1=combined[idx][0,0][0,0][0,0]-mean
            d2=toCombined[idx][0,0][0,0][0,0]-mean

            combined[idx][0,0][0,0][0,0]=copy.deepcopy(mean)
            combined[idx][0,0][0,0][0,1]=(combined[idx][0,1]*(combined[idx][0,0][0,0][0,1]+d1**2)+toCombined[idx][0,1]*(toCombined[idx][0,0][0,0][0,1]+d2**2))/(combined[idx][0,1]+toCombined[idx][0,1])
            ...
            if fullSet2[0,0]==None:
                ...
                fullSet2=copy.deepcopy(combined.reshape(-1,14))
                if toAddIdx[0,0]!=None:
                    for toi in toAddIdx[0]:
                        toAdd=fullSet1[toi]
                        toAdd[idx]=combined[idx]
                        fullSet2=np.concatenate((fullSet2,toAdd.reshape(-1,14)),axis=0)
                
            else:
                
                fullSet2=np.concatenate((fullSet2,combined.reshape(-1,14)),axis=0)
                if toAddIdx[0,0]!=None:
                    for toi in toAddIdx[0]:
                        toAdd=fullSet1[toi]
                        toAdd[idx]=combined[idx]
        ...
        for j in range(i+1,lenTotal):
            ...
            if i!=j:
                ...
                # user state  
                    ...
                    fullSet1=np.concatenate((fullSet1, rule2.reshape(-1,14)),axis=0)
...
def nodeRange(nearNodes,rule):
    ...
    for i in range(lenNearNode):
        ...
        if nearNodes[i][1]>=rule[0,1] and nearNodes[i][2]>=rule[0,2]:
            ...
            mean=(nearNodes[i][0]*nearNodes[i][4][0,0]+1*rule[0,0][0,0])/(nearNodes[i][0]+1)

            d1=nearNodes[i][4][0,0]-mean 

            d2=rule[0,0][0,0]-mean

            nearNodes[i][4][0,0]=copy.deepcopy(mean)

            nearNodes[i][4][0,1]=(nearNodes[i][0]*(nearNodes[i][4][0,1]+d1**2)+1*(rule[0,0][0,1]+d2**2))/(nearNodes[i][0]+1)

            resultRangeInt[0,0]=nearNodes[i][4]
...
class Node:
    ...
    def checkNode(self,rule,checkType):
        ...
        for node in range(length):
            ...
            if checkType==0:
                ...
                if self.subNodes[node][0].data[0,1]==rule[0,1] or self.subNodes[node][0].data[0,2]==rule[0,2]:
                    ...
                    mean=(self.subNodes[node][1]*self.subNodes[node][0].data[0,0][0,0]+1*rule[0,0][0,0])/(self.subNodes[node][1]+1)

                    d1=self.subNodes[node][0].data[0,0][0,0]-mean 

                    d2=rule[0,0][0,0]-mean

                    self.subNodes[node][0].data[0,0][0,0]=copy.deepcopy(mean)

                    self.subNodes[node][0].data[0,0][0,1]=(self.subNodes[node][1]*(self.subNodes[node][0].data[0,0][0,1]+d1**2)+1*(rule[0,0][0,1]+d2**2))/(self.subNodes[node][1]+1)
        ...
        if checkType!=0:
            ...
            min_dis=1000*calculateEpsilon(checkType, dataset)
            ...
            if checkType!=0:
                ...
                if (abs(self.subNodes[node][0].data[0,1]-rule[0,1])<=torch.tensor([[min_dis]]).type(torch.float32) or abs(self.subNodes[node][0].data[0,2]-rule[0,2])<=torch.tensor([[min_dis]])).type(torch.float32) or (rule[0,1]>=self.subNodes[node][0].data[0,1] and rule[0,1]<=self.subNodes[node][0].data[0,2]):
                    ...
                    if nearNodes[0][0]==None:
                        ...
                        nearNodeInter=np.full([1,5],None)
        ...
        if lengthNode!=None and lengthNode>1:
            ...
            for i in range(1, lengthNode):
                ...
                if abs(combined[0].data[0,1]-self.subNodes[i][0].data[0,1])<torch.tensor([[min_dis]]).type(torch.float32) or abs(combined[0].data[0,2]-self.subNodes[i][0].data[0,1])<torch.tensor([[min_dis]]).type(torch.float32) or abs(combined[0].data[0,1]-self.subNodes[i][0].data[0,2])<torch.tensor([[min_dis]]).type(torch.float32) or abs(combined[0].data[0,2]-self.subNodes[i][0].data[0,2])<torch.tensor([[min_dis]]).type(torch.float32): 
                    ...
                    mean=(combined[1]*combined[0].data[0,0][0,0]+self.subNodes[i][1]*self.subNodes[i][0].data[0,0][0,0])/(combined[1]+self.subNodes[i][1])

                    d1=combined[0].data[0,0][0,0]-mean
                    d2=self.subNodes[i][0].data[0,0][0,0]-mean

                    combined[0].data[0,0][0,0]=copy.deepcopy(mean)
                    combined[0].data[0,0][0,1]=(combined[1]*(combined[0].data[0,0][0,1]+d1**2)+self.subNodes[i][1]*(self.subNodes[i][0].data[0,0][0,1]+d2**2))/(combined[1]+self.subNodes[i][1])
    ...
    def refineRule(self, checkType=0, add=0):
        ...
        for i in range(14):
            ...
            # if i==187:
            #     print('yes')
            fullSet=sortRules(fullSet,13-i)
