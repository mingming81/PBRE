    ...
    def checkNode(self,rule,checkType):
        ...
        for node in range(length):
            ...
            if checkType!=0 and checkType!=1 and checkType!=2:
                ...
                if (abs(self.subNodes[node][0].data[0,1]-rule[0,1])<=torch.tensor([[0.01]])*1000 or abs(self.subNodes[node][0].data[0,2]-rule[0,2])<=torch.tensor([[0.01]])*1000) or (rule[0,1]>=self.subNodes[node][0].data[0,1] and rule[0,1]<=self.subNodes[node][0].data[0,2]):
                    case=1
