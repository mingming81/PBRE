...
class Node:
    ...
    def checkNode(self,rule,checkType):
        ...
        for node in range(length):
            ...
            if checkType!=0 and checkType!=1 and checkType!=2:
                ...
                if (abs(self.subNodes[node][0].data[0,1]-rule[0,1])<=torch.tensor([[100]]) or abs(self.subNodes[node][0].data[0,2]-rule[0,2])<=torch.tensor([[100]])) or (rule[0,1]>=self.subNodes[node][0].data[0,1] and rule[0,1]<=self.subNodes[node][0].data[0,2]):
                    case=1
    ...
    def getSubNodes(self,node0):
        ...
        stateTypeRuleSet=np.full([1,4],None)
    ...
    def refineRule(self):
        ...
        fullSetDup=sorted(fullSetDup,key=lambda x: x[2][0,0][0,1])
    
        fullSetDup=np.array(fullSetDup)
