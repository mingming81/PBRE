...
# np.save(other_data_name+'/sonar_x_train_5.npy', X_train)
# np.save(other_data_name+'/sonar_y_train_5.npy', y_train)
# np.save(other_data_name+'/sonar_x_test_5.npy', X_test)
# np.save(other_data_name+'/sonar_y_test_5.npy', Y_test)
# np.save(other_data_name+'/sonar_x_val_5.npy', X_val)
# np.save(other_data_name+'/sonar_y_val_5.npy', Y_val)
...
# notsignificant=np.array([16,29,17,40,24,39,37,31,38,14,59,30,6,55,5,52,41,13,57,7,2, 21, 36,4,19,34,  1,20,3,0,43,46,45,12,8, 44,11 ])
# notsignificant=np.array([None])
...
def getCorrelation(x, arraySet):
    ...
    # dataset=data.drop([data.keys()[i] for i in notsignificant], axis=1)
...
def getSpecies(y_pred,acc_rule, x):
    ...
    # notsignificant=np.array([14,15,16,17,23,24,25,26,27,28,29,31,37,38,39,40,54,56,59])
    ...
    if num_rules==1:
        ...
        idx_toDecide=np.array(idx_toDecide).reshape(-1,61)
...
# indexes=np.array([i for i in range(lenX)])
...
# %%
# F
max_acc=np.load(other_data_name+'/sonar_test_acc_max.npy')  
notsignificant=np.load(other_data_name+'/sonar_not_signigicant_B.npy').astype('float32')  
...
notsignificant=np.load(other_data_name+'/sonar_not_signigicant_F.npy').astype('float32') 
# %%
# B
...
# np.save(other_data_name+'/sonar_test_acc_max.npy',max_acc)  
# np.save(other_data_name+'/sonar_not_signigicant_B.npy',notsignificant)    

# np.save(other_data_name+'/sonar_not_signigicant_F.npy',notsignificant)    
