...
# %%
dataset = pd.read_csv('./../v3/data2/sonar.all-data.csv', header=None)



dataset= dataset.replace(to_replace="R", value=0, regex=True)
dataset= dataset.replace(to_replace="M", value=1, regex=True)
...
# print(f'dataset check: {dataset[key_val[2]].values}')

# values=sorted(dataset[key_val[2]].values, key=lambda x: x)

# print('==================================================')

# print(f'dataset check: {values}')
    ...
    for i in range(1,fullSet1.shape[0]):
        ...
        if toCombined[idx][0,0][0,1]<=combined[idx][0,0][0,2]:
            ...
            if fullSet2[0,0]==None:
                ...
                fullSet2=copy.deepcopy(combined.reshape(-1,61))
                if toAddIdx[0,0]!=None:
                    for toi in toAddIdx[0]:
                        toAdd=fullSet1[toi]
                        toAdd[idx]=combined[idx]
                        fullSet2=np.concatenate((fullSet2,toAdd.reshape(-1,61)),axis=0)
                
            else:
                
                fullSet2=np.concatenate((fullSet2,combined.reshape(-1,61)),axis=0)
                if toAddIdx[0,0]!=None:
                    for toi in toAddIdx[0]:
                        toAdd=fullSet1[toi]
                        toAdd[idx]=combined[idx]
        ...
        for j in range(i+1,lenTotal):
            ...
            if i!=j:
                ...
                # user state  
                    ...
                    fullSet1=np.concatenate((fullSet1, rule2.reshape(-1,61)),axis=0)
...
class Node:
    ...
    def checkNode(self,rule,checkType):
        ...
        if checkType!=0:
            ...
            importAtt=np.array([i for i in range(1,61)])
    ...
    def update(self,toCheck,checkType=0,add=0):
        ...
        # if checkType==0:
        #     rule[0,2]=np.array([[rule[0,2]]])
        # else:
        # rule[0,1]=torch.from_numpy(rule[0,1]).reshape(1,-1).type(torch.float32)
    ...
    def refineRule(self, checkType=0, add=0):
        ...
        for i in range(61):
            ...
            # if i==187:
            #     print('yes')
            fullSet=sortRules(fullSet,60-i)
