# this version adds one hidden layer in the neural networks, and adds the condition that the curtain should be closed when the user is sleeping, considers only the user state as inputs

# v1: hihdden layer 10
# v2: hidden layers 10+10
# v3: hiddens 8+16, add cur=0 at user=3, add break
...
for epoch in range(num_epochs):
    ...
    if epoch%100==0 and epoch!=0:      
        torch.save(LightService.LightServiceModel.state_dict(),dirName+f'/single_service_light_v3_{32+1+int(epoch/100)}_v9.pth')
        np.save(other_data_name+f'/seenTrainingStates_v3_{32+1+int(epoch/100)}_v9.npy',seenTrainingStates)
        np.save(other_data_name+f'/totalEpochRewards_v3_{32+1+int(epoch/100)}_v9.npy',totalEpochRewards)
        seenTrainingStates=deque(maxlen=100000)


torch.save(LightService.LightServiceModel.state_dict(),dirName+f'/single_service_light_v3_v9.pth')
